...
(define-public (is-dao-or-extension)
  (ok (asserts! (or (is-eq tx-sender .dao) (contract-call? .dao is-extension contract-caller)) ERR_UNAUTHORIZED))
)
...
(define-public (add-proposal (proposal <proposal-trait>) (data {startBlockHeight: uint, endBlockHeight: uint, proposer: principal}))
  (begin
    (try! (is-dao-or-extension))
    (asserts! (is-none (contract-call? .dao executed-at proposal)) ERR_PROPOSAL_ALREADY_EXECUTED)
...
(define-public (conclude (proposal <proposal-trait>))
  (let
    ...
    (and passed (try! (contract-call? .dao execute proposal tx-sender)))
    (ok passed)
  )
)
