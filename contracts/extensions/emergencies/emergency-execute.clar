...
(define-data-var executiveTeamSunsetHeight uint (+ block-height u4380))
...
(define-public (executive-action (proposal <proposal-trait>))
	(let
		...
		(asserts! (is-none (contract-call? .core-dao executed-at proposal)) ERR_ALREADY_EXECUTED)
		...
		(and (>= signals (var-get executiveSignalsRequired))
			(try! (contract-call? .core-dao execute proposal tx-sender))
		)
