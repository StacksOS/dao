...
(define-map Claims uint { amount: uint, claimedAt: uint, member: principal, roundId: uint })
...
(define-public (start)
  (begin
    ...
    (let
      (
        (nextRoundId (+ u1 (var-get roundId)))
      )
...
(define-public (ragequit (membershipTokenId uint))
  (begin
    ...
    (try! (contract-call? .treasury stx-transfer (get-payout tx-sender) tx-sender none))
